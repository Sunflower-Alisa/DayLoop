...
console.log('QuestionDetail setup ran')
const route = useRoute()
const router = useRouter()
const isNew = computed(() => route.name === 'question-new')
...
const form = ref({
  ...
  content: '',
  answer: '',
  answer_source: 'self' as 'self' | 'ai' | 'web',
  category: '',
  tags: '',
})
...
onMounted(async () => {
  try {
    categories.value = await api.getQuestionCategories()
  } catch (e) {
    alert('加载分类失败: ' + (e as Error).message)
  }
  if (isNew.value) {
    loading.value = false
    return
  ...
  try {
    ...
    form.value = {
      ...
      content: q.content,
      answer: q.answer,
      answer_source: q.answer_source || 'self',
      category: q.category,
      tags: q.tags || '',
    }
    extractImages(q.content + q.answer)
  } catch (e) {
    alert('加载问题失败: ' + (e as Error).message)
  }
  loading.value = false
})
...
async function insertImage(target: 'content' | 'answer') {
...
function removeImage(url: string) {
  ...
  form.value.answer = form.value.answer.replace(regex, '')
  extractImages(form.value.content + form.value.answer)
}
...
async function save() {
  ...
  saving.value = true
  if (isNew.value) {
    const q = await api.createQuestion(form.value)
    router.replace('/questions/' + q.id)
  } else {
    await api.updateQuestion(question.value!.id, form.value)
    question.value = await api.getQuestion(question.value!.id)
  }
  saving.value = false
}
...
<template>
  <div class="question-detail">
    <button class="back-btn" @click="router.push('/questions')">← 返回</button>
    ...
    <template v-else>
      <div class="form-group">
        <label>问题</label>
        <input v-model="form.title" placeholder="输入问题" />
      </div>
      <div class="form-group">
        <label>分类</label>
        <div class="cat-row">
      ...
      <div class="form-group">
        <label>标签 <span class="text-muted">（逗号分隔，如：数学,面试）</span></label>
        <input v-model="form.tags" placeholder="标签1,标签2,标签3" />
      </div>
      <div class="form-group">
        <label>问题详情 <span class="text-muted">（支持文字和图片）</span></label>
        <div class="editor-toolbar">
          <button class="toolbar-btn" @click="insertImage('content')" :disabled="uploading" title="插入图片">
            {{ uploading ? '⏳ 上传中...' : '🖼️ 图片' }}
          </button>
      ...
      <div class="form-group">
        <label>回答来源</label>
        <select v-model="form.answer_source">
          <option value="self">自答</option>
          <option value="ai">AI 回答</option>
      ...
      <div class="form-group">
        <label>回答</label>
        <div class="editor-toolbar">
          <button class="toolbar-btn" @click="insertImage('answer')" :disabled="uploading" title="插入图片">
            {{ uploading ? '⏳ 上传中...' : '🖼️ 图片' }}
          </button>
        </div>
        <textarea v-model="form.answer" placeholder="输入回答" rows="8"></textarea>
      </div>
      ...
      <div v-if="question?.linked_tasks?.length" class="linked-section">
        <h3>🔗 关联任务 ({{ question.linked_tasks.length }})</h3>
        <div v-for="task in question.linked_tasks" :key="task.id" class="linked-task-card">
          <div class="lt-header">
      ...
      <div class="form-actions">
        <button v-if="!isNew" class="btn btn-danger" @click="deleteQuestion">删除</button>
