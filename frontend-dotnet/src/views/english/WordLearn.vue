...
const timer = new StudyTimer('words')
...
async function submit(r: { correct: boolean; know?: boolean }) {
  ...
  if (!w) return
  try {
    await englishApi.submitLearn({ word_id: w.id, correct: r.correct, know: r.know, is_review: false })
  } catch (e) {}
}
...
function finish() {
  ...
  sessionSaved = true
  englishApi.saveSession('words', timer.stamp(), timer.stamp(), timer.seconds())
}

function goHome() { router.push({ name: 'english-words' }) }
function goReview() { router.push({ name: 'english-words-review' }) }

onMounted(async () => {
  try {
    const task = await englishApi.getDaily()
    queue.value = task.new_words
  } catch (e) {}
  loadDone.value = true
  ...
  <div class="learn-page">
    ...
    <template v-else-if="phase === 'done'">
      <div class="goal-card">
        ...
        <div class="goal-title">本轮学习完成</div>
        <div class="goal-sub">共学习 {{ queue.length }} 个新词</div>
        <div class="goal-stats">
          <div class="stat"><div class="num ok">+{{ stats.correct + stats.know }}</div><div class="label">掌握/答对</div></div>
          <div class="stat"><div class="num bad">{{ stats.wrong }}</div><div class="label">有待复习</div></div>
          <div class="stat"><div class="num">{{ stats.know }}</div><div class="label">太简单跳过</div></div>
        </div>
        <div class="goal-actions">
          <button class="btn btn-ghost" @click="goReview">马上复习</button>
          <button class="btn btn-primary" @click="goHome">完成</button>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="current">
      <div class="topbar">
        ...
        <div class="e-progress topbar-progress">
          <div class="e-progress-fill" :style="{ width: progressPct + '%' }"></div>
        </div>
      ...
      <div v-if="phase === 'study'" class="study-wrap">
        <div class="word-card">
          <div class="img-ban" :style="{ background: '#4f46e5' }">
      ...
      <div v-else-if="phase === 'quiz' && quiz" class="quiz-wrap">
        <div class="quiz-question">
          ...
          <div v-if="quiz.type === 'word'" class="q-hint">{{ quiz.prompt }}</div>
          <div v-if="quiz.type === 'meaning'" class="q-hint">{{ quiz.prompt }}</div>
        </div>
        <div class="options">
          <button
        ...
        <div v-if="pickedIndex !== null" class="quiz-feedback">
          <div :class="['fb', pickedIndex === quiz.optionIndex ? 'ok' : 'no']">
            {{ pickedIndex === quiz.optionIndex ? '✓ 正确！' : '✗ 正确是：' + quiz.answer }}
          </div>
      ...
      <div v-if="phase === 'study'" class="study-actions">
        <button class="btn btn-ghost" @click="markKnow">😄 太简单了</button>
...
.fall-letter { color: #fff; font-size: 90px; font-weight: 800; }
